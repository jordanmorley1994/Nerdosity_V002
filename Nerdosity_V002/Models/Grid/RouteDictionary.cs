...
namespace Nerdosity_V002.Models
{
    ...
    public class RouteDictionary : Dictionary<string, string>
    {
        ...
        public string ManufacturerFilter
        {
            get
            {
                string s = Get(nameof(ProductsGridDTO.Manufacturer))?.Replace(FilterPrefix.Manufacturer, "");
                int index = s?.IndexOf('-') ?? -1;
                return (index == -1) ? s : s.Substring(0, index);
            }
            set => this[nameof(ProductsGridDTO.Manufacturer)] = value;
        }

        public void ClearFilters() =>
            CategoryFilter = PriceFilter = ManufacturerFilter = ProductsGridDTO.DefaultFilter;
